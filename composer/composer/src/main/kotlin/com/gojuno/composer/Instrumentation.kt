...
class InstrumentationException(message: String) : RuntimeException(message)

private fun String.throwIfError(output: File) = when {
  contains("INSTRUMENTATION_RESULT: shortMsg=") -> {
    throw InstrumentationException("Application process crashed. Check Logcat output for more details.")
  }
...
private fun parseInstrumentationEntry(str: String): InstrumentationEntry =
    InstrumentationEntry(
        ...
        statusCode = str.substringBetween(
            ...
            .let { statusCode ->
              when (statusCode) {
                null -> throw IllegalStateException(
                    "Unknown test status code [$statusCode], " +
                    "please report that to Composer maintainers $str"
                )
                else -> statusCode
              }
            },
...
fun readInstrumentationOutput(output: File): Observable<InstrumentationEntry> {
  data class Result(
      val buffer: String = "",
      val readyForProcessing: Boolean = false
  )
...
fun Observable<InstrumentationEntry>.asTests(): Observable<InstrumentationTest> {
  data class Result(
      val entries: List<InstrumentationEntry> = emptyList(),
      val tests: List<InstrumentationTest> = emptyList(),
      ...
      .scan(Result()) { previousResult, newEntry ->
        ...
        Result(
            entries = entries.filter { entry ->
              tests.firstOrNull {
                it.className == entry.clazz &&
                it.testName == entry.test
              } == null
            },
            tests = tests,
